...
`check` compares each configured `wordpress.ts` with the contract published by WordPress. It exits non-zero
and prints a unified diff when a committed client is stale. It never changes the file.
...
With [`test.local`](#configuration) set, `kizlo test` runs your test suite against **real WordPress**, with no
...
| `test.fixtures` | `test` | Fixtures to install and seed onto test WordPress: core content plus any integration's fixtures |
