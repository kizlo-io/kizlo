...
server), returning the `{ success, data, error }` envelope your frontend sees.
...
It complements `getKizloTestInstance`: that calls procedures directly through the server `client` (the quickest
check of what a procedure *does*); this goes through the full client → handler path, catching what a direct call
