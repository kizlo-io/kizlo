...
At `src/lib/kizlo/server/index.ts`. `createKizlo()` reads your credentials from `process.env` on the
TanStack Start server, so there's nothing to pass:
...
VITE_KIZLO_API_URL=http://localhost:3000/api/kizlo
```
...
That's it 🎉 — here's the payoff: a complete dynamic blog post route. A server function fetches the post
...
// Return only what the page renders (title, content, SEO) — a lean, serializable payload, not the
