...
Cart and checkout requests carry identity headers (`X-Kizlo-User-Id`, `X-Kizlo-Guest-Token`, and their supported alternatives) that the headless session handler validates before selecting a cart. When a user signs in with a guest cart, the plugin merges line items during that original request. It does not merge coupons, addresses, or shipping selections. Rejected guest lines are returned in the Store API cart's existing `errors` field.
