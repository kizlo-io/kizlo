...
export function DeleteRegistration({
	...
	slug: string
	/** The object's plural label, e.g. "Books". Used in the modal copy. */
	name: string
	/** Card + button heading, served from the nav (e.g. "Delete Books"). */
	label: string
	/** Card description, served from the nav. */
	desc: string
	onDeleted: () => void
}) {
	...
	return (
		<>
			<div className="flex items-center justify-between gap-4">
				<FieldLabel label={label} desc={desc} descMode="below" />
				<Button type="button" variant="secondary" className="shrink-0 text-red-700!" onClick={() => setOpen(true)}>
					Delete
				</Button>
			</div>
						...
						{progress ? (
							...
							<>
								<p className="m-0 text-neutral-700 text-sm">
									This permanently removes <strong>{name}</strong>. There is no trash. Choose what happens to its {noun}.
								</p>
