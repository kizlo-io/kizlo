/* eslint-disable */

// @ts-nocheck

// noinspection JSUnusedGlobalSymbols

// This file was automatically generated by TanStack Router.
// You should NOT make any changes in this file as it will be overwritten.
// Additionally, you should also exclude this file from your linter and/or formatter to prevent it from being checked or modified.

import { Route as rootRouteImport } from "./routes/__root"
import { Route as ApiKizloSplatRouteImport } from "./routes/api/kizlo/$"
import { Route as BlogSlugRouteImport } from "./routes/blog/$slug"
import { Route as IndexRouteImport } from "./routes/index"
import { Route as RobotsDottxtRouteImport } from "./routes/robots[.]txt"
import { Route as SiteDotwebmanifestRouteImport } from "./routes/site[.]webmanifest"
import { Route as SitemapDotxmlRouteImport } from "./routes/sitemap[.]xml"
import { Route as SitemapsSplatRouteImport } from "./routes/sitemaps/$"

const IndexRoute = IndexRouteImport.update({
	id: "/",
	path: "/",
	getParentRoute: () => rootRouteImport,
} as any)
const RobotsDottxtRoute = RobotsDottxtRouteImport.update({
	id: "/robots.txt",
	path: "/robots.txt",
	getParentRoute: () => rootRouteImport,
} as any)
const SiteDotwebmanifestRoute = SiteDotwebmanifestRouteImport.update({
	id: "/site.webmanifest",
	path: "/site.webmanifest",
	getParentRoute: () => rootRouteImport,
} as any)
const SitemapDotxmlRoute = SitemapDotxmlRouteImport.update({
	id: "/sitemap.xml",
	path: "/sitemap.xml",
	getParentRoute: () => rootRouteImport,
} as any)
const BlogSlugRoute = BlogSlugRouteImport.update({
	id: "/blog/$slug",
	path: "/blog/$slug",
	getParentRoute: () => rootRouteImport,
} as any)
const SitemapsSplatRoute = SitemapsSplatRouteImport.update({
	id: "/sitemaps/$",
	path: "/sitemaps/$",
	getParentRoute: () => rootRouteImport,
} as any)
const ApiKizloSplatRoute = ApiKizloSplatRouteImport.update({
	id: "/api/kizlo/$",
	path: "/api/kizlo/$",
	getParentRoute: () => rootRouteImport,
} as any)

export interface FileRoutesByFullPath {
	"/": typeof IndexRoute
	"/robots.txt": typeof RobotsDottxtRoute
	"/site.webmanifest": typeof SiteDotwebmanifestRoute
	"/sitemap.xml": typeof SitemapDotxmlRoute
	"/blog/$slug": typeof BlogSlugRoute
	"/sitemaps/$": typeof SitemapsSplatRoute
	"/api/kizlo/$": typeof ApiKizloSplatRoute
}
export interface FileRoutesByTo {
	"/": typeof IndexRoute
	"/robots.txt": typeof RobotsDottxtRoute
	"/site.webmanifest": typeof SiteDotwebmanifestRoute
	"/sitemap.xml": typeof SitemapDotxmlRoute
	"/blog/$slug": typeof BlogSlugRoute
	"/sitemaps/$": typeof SitemapsSplatRoute
	"/api/kizlo/$": typeof ApiKizloSplatRoute
}
export interface FileRoutesById {
	__root__: typeof rootRouteImport
	"/": typeof IndexRoute
	"/robots.txt": typeof RobotsDottxtRoute
	"/site.webmanifest": typeof SiteDotwebmanifestRoute
	"/sitemap.xml": typeof SitemapDotxmlRoute
	"/blog/$slug": typeof BlogSlugRoute
	"/sitemaps/$": typeof SitemapsSplatRoute
	"/api/kizlo/$": typeof ApiKizloSplatRoute
}
export interface FileRouteTypes {
	fileRoutesByFullPath: FileRoutesByFullPath
	fullPaths: "/" | "/robots.txt" | "/site.webmanifest" | "/sitemap.xml" | "/blog/$slug" | "/sitemaps/$" | "/api/kizlo/$"
	fileRoutesByTo: FileRoutesByTo
	to: "/" | "/robots.txt" | "/site.webmanifest" | "/sitemap.xml" | "/blog/$slug" | "/sitemaps/$" | "/api/kizlo/$"
	id: "__root__" | "/" | "/robots.txt" | "/site.webmanifest" | "/sitemap.xml" | "/blog/$slug" | "/sitemaps/$" | "/api/kizlo/$"
	fileRoutesById: FileRoutesById
}
export interface RootRouteChildren {
	IndexRoute: typeof IndexRoute
	RobotsDottxtRoute: typeof RobotsDottxtRoute
	SiteDotwebmanifestRoute: typeof SiteDotwebmanifestRoute
	SitemapDotxmlRoute: typeof SitemapDotxmlRoute
	BlogSlugRoute: typeof BlogSlugRoute
	SitemapsSplatRoute: typeof SitemapsSplatRoute
	ApiKizloSplatRoute: typeof ApiKizloSplatRoute
}

declare module "@tanstack/solid-router" {
	interface FileRoutesByPath {
		"/": {
			id: "/"
			path: "/"
			fullPath: "/"
			preLoaderRoute: typeof IndexRouteImport
			parentRoute: typeof rootRouteImport
		}
		"/robots.txt": {
			id: "/robots.txt"
			path: "/robots.txt"
			fullPath: "/robots.txt"
			preLoaderRoute: typeof RobotsDottxtRouteImport
			parentRoute: typeof rootRouteImport
		}
		"/site.webmanifest": {
			id: "/site.webmanifest"
			path: "/site.webmanifest"
			fullPath: "/site.webmanifest"
			preLoaderRoute: typeof SiteDotwebmanifestRouteImport
			parentRoute: typeof rootRouteImport
		}
		"/sitemap.xml": {
			id: "/sitemap.xml"
			path: "/sitemap.xml"
			fullPath: "/sitemap.xml"
			preLoaderRoute: typeof SitemapDotxmlRouteImport
			parentRoute: typeof rootRouteImport
		}
		"/blog/$slug": {
			id: "/blog/$slug"
			path: "/blog/$slug"
			fullPath: "/blog/$slug"
			preLoaderRoute: typeof BlogSlugRouteImport
			parentRoute: typeof rootRouteImport
		}
		"/sitemaps/$": {
			id: "/sitemaps/$"
			path: "/sitemaps/$"
			fullPath: "/sitemaps/$"
			preLoaderRoute: typeof SitemapsSplatRouteImport
			parentRoute: typeof rootRouteImport
		}
		"/api/kizlo/$": {
			id: "/api/kizlo/$"
			path: "/api/kizlo/$"
			fullPath: "/api/kizlo/$"
			preLoaderRoute: typeof ApiKizloSplatRouteImport
			parentRoute: typeof rootRouteImport
		}
	}
}

const rootRouteChildren: RootRouteChildren = {
	IndexRoute: IndexRoute,
	RobotsDottxtRoute: RobotsDottxtRoute,
	SiteDotwebmanifestRoute: SiteDotwebmanifestRoute,
	SitemapDotxmlRoute: SitemapDotxmlRoute,
	BlogSlugRoute: BlogSlugRoute,
	SitemapsSplatRoute: SitemapsSplatRoute,
	ApiKizloSplatRoute: ApiKizloSplatRoute,
}
export const routeTree = rootRouteImport._addFileChildren(rootRouteChildren)._addFileTypes<FileRouteTypes>()
