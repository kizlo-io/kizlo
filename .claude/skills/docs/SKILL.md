...
## Never use em-dashes (strict)

Do not use the em-dash character (`—`) anywhere, ever. Not in prose, not in bullets, not in headings, not in frontmatter descriptions, not in callouts. There is no "house style" exception and no aside that justifies one. A single `—` in the output is a defect to fix before reporting done.

Rewrite every aside another way:

- End the thought and start a new sentence with a period.
- Use a comma for a light pause, a colon to introduce a list or explanation, or parentheses for a true aside.
- In a bulleted term definition, put a colon after the bold term (`**\`router\`**: the procedures...`), not a dash.

The `→` arrow inside code comments (`// event.data → { post_id, post_type }`) is fine. Only `—` is banned. Avoid en-dashes (`–`) too; use a plain hyphen for ranges (`3-4`, `20-40%`).
...
- **No fake enthusiasm.** Don't manufacture excitement. And per the rule above, never reach for an em-dash to add drama; a period works better.
...
2. **Cut fluff**, the sentences and words that add no meaning (rule 2). This usually removes 20-40% of the text.
...
6. **Fix correctness**, including stale commands, paths, flags, and examples.
7. **Purge em-dashes.** Existing `—` in a page you touch is not "house style" to preserve; remove every one per the strict rule above, even the ones you didn't write.
...
- [ ] **Zero em-dashes (`—`) anywhere**, including in pages you only edited. En-dashes avoided too.
