...
import type { AnyIntegration } from "../shared/integration"
 ...
 */
export async function getKizloClientTestInstance<TIntegrations extends readonly AnyIntegration[]>(
	kizlo: Kizlo<TIntegrations>,
	options?: { fetch?: (request: Request) => Promise<Response>; url?: string },
) {
	const contract = await generateContract(kizlo.procedures)

	return new KizloClient({
		url: options?.url ?? "http://test.local",
		fetch: options?.fetch ?? ((req) => kizlo.handler(req)),
		contract: contract as unknown as Kizlo<TIntegrations>["procedures"],
	})
}
