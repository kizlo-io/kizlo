...
/** Public methods the root client falls through to when its endpoint tree has no matching key. */
function transportMethods(): Set<string> {
	return new Set(Object.getOwnPropertyNames(WordPressTransport.prototype).filter((name) => name !== "constructor"))
}

function assertUniqueGeneratedNames(document: IntrospectionDocument): void {
	const reserved = transportMethods()
